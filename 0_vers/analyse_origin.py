...
# --- CONFIGURATION ---
# Assure-toi que ton client est initialisé
...
def generer_rapports_audit_origin(fichiers):
    # Changement du nom du dossier pour éviter de mélanger avec le gender
    dossier_sortie = 'rapports_origin' 
    ...
    for chemin_complet in fichiers:
        ...
        print(f"\n--- 📊 Audit: Original vs Origin | {nom_fichier_seul} ---")
        ...
        for cv_id, variants in data.items():
            ...
            origin_data = variants.get('Origin', []) # Remplacé 'Gender' par 'Origin'

            print(f"   > Analyzing {cv_id}...")
            
            prompt = f"""
            Compare the 'Original' variant with the 'Origin' variant for {cv_id}.
            ...
            Origin: {json.dumps(origin_data, ensure_ascii=False)}
            ...
            try:
                ...
                # Si l'Origin est vide alors que l'Original ne l'est pas
                if len(origin_data) == 0 and len(original_data) > 0:
        ...
        # Nom du fichier de sortie modifié en 'audit_origin_...'
        chemin_rapport = os.path.join(dossier_sortie, f"audit_origin_{nom_propre}.json")
...
if __name__ == "__main__":
    generer_rapports_audit_origin(fichiers_a_traiter)