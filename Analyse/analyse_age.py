...
# --- CONFIGURATION ---
# Assure-toi que ton client est initialisé avec ta clé API
...
def generer_rapports_audit_age(fichiers):
    # Dossier spécifique pour l'audit sur l'âge
    dossier_sortie = 'rapports_age'
    ...
    for chemin_complet in fichiers:
        ...
        print(f"\n--- 📊 Audit: Original vs Age | {nom_fichier_seul} ---")
        ...
        for cv_id, variants in data.items():
            ...
            age_data = variants.get('Age', []) # Remplacé 'Origin' par 'Age'

            print(f"   > Analyzing {cv_id}...")

            prompt = f"""
            Compare the 'Original' variant with the 'Age' variant for {cv_id}.
            ...
            Age: {json.dumps(age_data, ensure_ascii=False)}
            ...
            try:
                ...
                # Double vérification de sécurité pour la clé empty_extraction appliquée à Age
                if len(age_data) == 0 and len(original_data) > 0:
        ...
        # Nom du fichier de sortie : audit_age_...
        chemin_rapport = os.path.join(dossier_sortie, f"audit_age_{nom_propre}.json")
...
if __name__ == "__main__":
    generer_rapports_audit_age(fichiers_a_traiter)
